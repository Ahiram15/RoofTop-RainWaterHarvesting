document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    map.on('load', () => {
      ...
      // We must re-add any custom layers and marker here.
      ...
      if (window._rwh_marker) {
        window._rwh_marker.addTo(map);
      } else {
        window._rwh_marker = new maplibregl.Marker({ draggable: true })
          .setLngLat([78.9629, 20.5937])
          .addTo(map);
      }
      // Re-add rooftop polygon if exists (add your custom layer logic here)
      if (window._rwh_rooftopPolygon && map.getSource('rooftop')) {
        map.getSource('rooftop').setData(window._rwh_rooftopPolygon);
      }
    });
  ...
  window.submitData = function() {
    ...
    formData = {
      ...
      propertyDetails: {
        ...
        openSpace: document.getElementById('openSpace').value,
        budgetRange: document.getElementById('budgetRange').value,
        storageMonths: document.getElementById('storageMonths').value
      },
      
      // Water sources
      waterSources: Array.from(document.querySelectorAll('input[type="checkbox"]:checked'))
                       .map(cb => cb.value),
    ...
    setTimeout(() => {
      ...
      // Navigate to results page (placeholder)
      alert('Assessment data collected successfully!\nRedirecting to assessment results...');
      
      // Reset button
      submitBtn.innerHTML = originalText;
      submitBtn.disabled = false;
      
      // Here you would typically redirect to the assessment results page
